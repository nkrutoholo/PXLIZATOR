...
function upload(target) {
    ...
    if (target.files &&
        ...
        document.getElementById('file1').alt = "Something went wrong! chinaaa";
    ...
    function view_image(image){
        ...
        reader.onload = function () {
            ...
            document.getElementById('label_size').textContent = "Size:  " +
...
function loaded_img(){
    document.getElementById('label_pixels').textContent = "Size in pxl: " +
...
async function sender() {
    ...
    if(image != 0 && check) {
        ...
        let response = await fetch('http://localhost:8080/pixelizator_war',{
        ...
        if (response.ok) {
            ...
            alert('НИМАГУ АТПРАВИТЬ САБчЕНИЕ ');
        }
    } else {
        alert("Check image");
    }
